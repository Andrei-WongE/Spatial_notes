...
$$ 
\begin{bmatrix} x & y \end{bmatrix}_{\text{Coordinates} \times  
\begin{bmatrix} 1 & 0 \\ 1.2 & 1 \end{bmatrix}}_{(1)}} \times  
\begin{bmatrix} \cos(\pi/20) & \sin(\pi/20)\\-\sin(\pi/20) & \cos(\pi/20) \end{bmatrix}_{(2)} + 
\begin{bmatrix} x\_add & y\_add \end{bmatrix}_{(3)}
$$
...
Mapping income groups in an interactive map.
...
# Set ESTR_INGPE and POB of less or equal than CERO to NA
population_sf <- population_sf |>
  mutate(ESTR_INGPE = ifelse(ESTR_INGPE > 0, ESTR_INGPE, NA)) |>
  mutate(POB = ifelse(POB > 0, POB, NA))

tmap_mode("view")
tm_shape(population_sf) +
    tm_borders() +
    tm_polygons("ESTR_INGPE"
                , colorNA = "grey75") +
    # tm_facets(ncol = 2, sync = NA) +
    tm_minimap() +
