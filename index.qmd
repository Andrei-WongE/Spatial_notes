...
ggplot() +
  #---The layer added later will come on top of the preceding layers---#
  geom_sf(data = Lima_multipolygons) +
  geom_sf(data = population_sf, aes(fill = POB)) +
  ggsn::blank()
...
```{comment}
Align equation with comments https://www.math.mcgill.ca/yyang/regression/RMarkdown/example.html
```
...
::: {.column-margin}
  ...
  Consider that $\pi/20 = 9\°$ and that $\cos(\pi/20) \approx 0.987$ width and $\sin(\pi/20) \approx 0.156$ height in an unit circle.
